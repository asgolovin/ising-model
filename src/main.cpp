...
int ising(int nb, int meas_sweeps, int method, FILE *fperror)
{
    ...
    for (long int t = 0; t < sweeps; t++){
        ...
        if ((t % block_size == 0) && (t > term_sweeps)){
            ...
            //printf("%10i: \t Energy = %.4f, \t Magnetization density = %.4f\n", t, energy_mem[pos], mag_mem[pos]);
            if (fabs(energy_mem[pos]) > 4) {
    ...
    for (int s = 0; s < num_meas; s++)
    {
        sigma_e    += (pow((e_JK[s] - e_mean), 2)                           * (num_meas - 1.)/num_meas);
        sigma_mag  += (pow((mag_JK[s] - mag_mean), 2)                       * (num_meas - 1.)/num_meas);
        sigma_heat += (pow(pow((e_JK[s] - e_mean), 2)*pow(size, 4)/(T*T) - heat_JK, 2) * (num_meas - 1.)/num_meas);
    ...
    printf("Average susceptibility:         %.6f +/- %.6f\n", susc_JK, sqrt(sigma_susc));
